...
$(function() {
  ...
  window.addEventListener('message', function(event) {
    ...
    if (Type == 'open') {
      ...
      document.execCommand("copy", false, event.data.data);
    } else if (Type == 'source') {
      MySource = Action
      console.log('Source set', MySource, Action)
    }
...
// Delete vehicle
function dv() {
  $.post('http://DokusCore--AdminMenu/dv', JSON.stringify({}))
}

// Toggle Noclip
function toggleNoclip() {
  $.post('http://DokusCore--AdminMenu/noclip', JSON.stringify({}))
}

function tpm() {
  $.post('http://DokusCore--AdminMenu/TPM', JSON.stringify({}))
}

// Toggle Ghost
function toggleGhost() {
  $.post('http://DokusCore--AdminMenu/ghost', JSON.stringify({}))
}

// Copy Coords
function coords() {
  $.post('http://DokusCore--AdminMenu/coords', JSON.stringify({}))
}

// Announce
function announce() {
  modalShow("announce", "Global Announce", "Enter a message below to send a global announcement to all players.", 0, true)
}

// Clear Area
function clearArea() {
  $.post('http://DokusCore--AdminMenu/clearArea', JSON.stringify({}))
}

// GodMode
function GodMode() {
  $.post('http://DokusCore--AdminMenu/GodMode', JSON.stringify({}))
}

// Direct message
function directMessage() {
  modalShow("directMessage", "Direct Message", "Send a player a direct message in text chat", 0, true)
}

// Goto player
function goto() {
  console.log('Source = ', MySource)
  $.post('http://DokusCore--AdminMenu/goto', JSON.stringify({
    player: players[selectedPlayer].id,
    MySource: MySource
  }))
}
...
// Bring player
function bring() {
  $.post('http://DokusCore--AdminMenu/bring', JSON.stringify({
    player: players[selectedPlayer].id
  }))
}
...
// Warn player
function warn() {
  modalShow("warn", "Warn Player", "Warn a player with a mesage. This message will be displayed for 1 minute", 1, true)
}
...
// Chain player
function chain() {
  $.post('http://DokusCore--AdminMenu/chain', JSON.stringify({
    player: players[selectedPlayer].id
  }))
}
...
// Spactate a player
function spectate() {
  $.post('http://DokusCore--AdminMenu/spectate', JSON.stringify({
    player: players[selectedPlayer].id,
    MySource: MySource
  }))
}
...
function UserSlap() {
  document.getElementById("gif-slap").style.display = "flex";
  setTimeout(function() {
    document.getElementById("gif-slap").style.display = "none";
  }, 10200);
}

// Slay player
function slay() {
  $.post('http://DokusCore--AdminMenu/slay', JSON.stringify({
    player: players[selectedPlayer].id
  }))
}
...
// Kick player
function kick() {
  modalShow("kick", "Kick Player", "Kick player from server. Enter your reason below.", 2, true)
}

// Ban player
function ban() {
  modalShow("ban", "Ban Player", "Ban player from server. Enter your reason below.", 2, true)
}
...
function modalShow(_action, _title, _body, _style, _input, _inputName, _inputArgs) {
  ...
  if (_input) {
    if (_inputArgs) {
      document.getElementById('modal-input').innerHTML = inputDropdown()
    } else {
      document.getElementById('modal-input').innerHTML = inputTextbox()
    }
  }

  // Set confirm button click
  if (_action != null) {
    document.getElementById('modal-confirm').setAttribute('onclick', `modalHide("${_action}")`)
  } else {
    document.getElementById('modal-confirm').setAttribute('onclick', `modalHide()`)
  }

  // Set confirm button text
  if (_inputName) {
    document.getElementById('modal-confirm').innerHTML = _inputName
  } else {
    document.getElementById('modal-confirm').innerHTML = "Confirm"
  }
...
function modalHide(_action) {
  ...
  if (typeof _action == "string") {
    if (data != "" || data != null) {
      $.post(`http://DokusCore--AdminMenu/${_action}`, JSON.stringify({
        data: data,
        player: players[selectedPlayer].id,
        MySource: MySource
      }))
    ...
    console.error(`action needs to be type: 'string'. Action was type: '${typeof _action}'`)
  }
