...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
RegisterNetEvent('DokusCore:Admin:Spectate', function()
  local PedID = PedID()
  SendNUIMessage({type = "close"})
  while IsSpectating do Wait(1)
    SetFreeze(PedID, IsSpectating)
    DisableFirstPersonCamThisFrame()
    local x, y, z = table.unpack(GetCoords(PedID))
    local h = TranslateHeading(PedID, GetEntityHeading(PedID))
    local Control = GetControls()
    local r = -h * math.pi / 180
    local dx = CurSpeed * math.sin(r)
    local dy = CurSpeed * math.cos(r)
    ...
    SetEntityCoordsNoOffset(PedID, x, y, z)
    SetEntityHeading(PedID, TranslateHeading(PedID, h))
