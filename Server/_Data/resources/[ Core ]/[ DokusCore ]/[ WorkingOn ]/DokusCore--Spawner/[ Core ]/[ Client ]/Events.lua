...
RegisterNetEvent('DokusCore:Spawner:User:Login', function()
  ...
  -- Continue
  TriggerEvent('DokusCore:Skins:Load:User')
...
RegisterNetEvent('DokusCore:Spawner:User:New', function()
  ...
  -- Is spawn area randomized ?
  if (Randomizer) then SpawnPos = Cutscenes[math.random(#Cutscenes)] end
  if (not (Randomizer)) then
    for k,v in pairs(Cutscenes) do
      if (Low(v.ID) == Low(StartPos)) then
        SpawnPos = v
      end
    end
  end
  ...
  local SP = SpawnPos.Coords.Start
  ...
  SetFreeze(PedID(), false)
  SetInvincible(PedID(), true)
  SetHeading(PedID(), SpawnPos.Coords.Heading)
  ...
  VehicleID = SpawnVehicle('STAGECOACH001X', SP, SpawnPos.Coords.Heading)
  ...
  DriveToCoords(DriverID, InVeh(DriverID, false), SpawnPos.Coords.End, 10.0, 1.0, GetMod, 67633207, 5.0, false)
  ...
  DriveToDestination(SpawnPos.Coords.End)
  ...
  WalkUserToTailor() Wait(5000) Cinema(false) DisplayHud(true) DisplayRadar(true)
  NetworkGhosting(PedID(), false)
end)
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
